...
export function MedidorTanque({ nome, combustivel, nivel, capacidade, nivelAlerta }: Props) {
  ...
  return (
    <div className="flex flex-col items-center gap-2.5 rounded-2xl border border-borda/50 bg-ardosia/30 p-3.5 shadow-sm transition-all duration-300 hover:border-borda hover:shadow-md hover:bg-ardosia/50 group select-none min-w-[125px]">
      <span className="text-[10px] font-bold uppercase tracking-wider text-suave">{nome}</span>
      
      {/* Corpo tridimensional do tanque */}
      <div className="relative h-48 w-16 overflow-hidden rounded-2xl border border-borda bg-gradient-to-b from-[color-mix(in_srgb,var(--color-claro)_4%,transparent)] to-[color-mix(in_srgb,var(--color-claro)_8%,transparent)] shadow-[inset_0_2px_8px_rgba(0,0,0,0.15)] flex items-center justify-center">
        ...
        <div className="absolute inset-y-0 left-0 w-full flex flex-col justify-between py-6 px-1.5 pointer-events-none opacity-40 z-10">
          {ticks.map((t) => (
            <div key={t} className="flex items-center justify-between w-full">
              <span className="w-1.5 h-[1px] bg-claro/50" />
      ...
      <div className="flex flex-col items-center leading-tight">
        ...
        <span className={`numeros text-lg font-extrabold mt-1.5 ${abaixoDoAlerta ? 'text-negativo font-black' : 'text-claro'}`}>
          {percentual}%
        </span>
      ...
      {abaixoDoAlerta && (
        <span className="rounded-full bg-negativo/10 border border-negativo/20 px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-negativo animate-pulse">
          crítico
        </span>
      )}
